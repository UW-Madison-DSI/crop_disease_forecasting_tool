...
![OSPO](OPENSOURDA_color-flush.png){ width=40% }
![PlantPath](PLANPATHCO_color-flush.png){ width=40% }
![PlantPath](DATASCIE_color-flush.png){ width=40% }
...
if (!is.null(params$tarspot) && nrow(params$tarspot) > 0) {
  ...
  tarspot_table <- params$tarspot %>%
    select(Date = date_day, 
           Risk, 
           Category = Risk_Class)%>%
    mutate(Risk = scales::percent(Risk*.01, accuracy = 1))
  
  # Render the table
  kable(
    tarspot_table,
    caption = "Risk Trend in the Seven Days Leading up to the Settled Forecast Date",
    format = "markdown"
  )
} else {
